...
const NavigationComponent = () => {
  ...
  return (
    <nav className="bg-sidebar h-16 px-10 flex justify-between items-center sticky top-0 z-20 border border-none border-b-sidebar-border text-sidebar-foreground shadow">
          ...
          <NavigationMenuList className="flex gap-6">
            {navLinks.map((link, index) => (
              <NavigationMenuItem key={index}>
                <NavigationMenuLink
                  href={link.href}
                  className={cn(index === 4 && "md:hidden")}
                >
                  {link.label}
                </NavigationMenuLink>
              </NavigationMenuItem>
      ...
      <div
        className={`w-30 absolute z-10 top-20 right-5 sm:right-10  ${navigationHamburger ? "block" : "hidden"}`}
      >
        <div className="bg-card flex flex-col rounded-lg justify-center items-center gap-2 py-2 shadow">
          {navLinks.map((link, index) => (
            <a
              ...
              href={link.href}
              className={cn(
                "text-sidebar-foreground underline-offset-4 hover:underline ",
                (index === 3 || index === 4) && "md:hidden",
              )}
            >
              {link.label}
            </a>
